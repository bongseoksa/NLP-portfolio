...
┌─────────────────────────────────────────────────────────┐
│  4. Vercel 자동 배포 (main 브랜치 push 시)             │
│     - GitHub push 이벤트 감지                            │
│     - Vercel 빌드 및 배포                                │
...
Vercel은 GitHub와 연동되어 **자동으로 배포**를 수행합니다. `main` 브랜치에 push할 때마다 자동으로 배포가 시작됩니다.
...
1. **Vercel 프로젝트 생성**
   ...
   - 프로젝트 설정: Framework Preset = "Other"

2. **환경 변수 설정**
   - Vercel 대시보드 → Settings → Environment Variables
   - 필수 변수: `VECTOR_FILE_URL`, `OPENAI_API_KEY`, `CLAUDE_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`
...
- [Vercel 배포 가이드](./02_Vercel_Deployment.md) - 자동 배포 설정, 환경 변수, 트러블슈팅
...
- [Vercel 배포 가이드](./02_Vercel_Deployment.md)
