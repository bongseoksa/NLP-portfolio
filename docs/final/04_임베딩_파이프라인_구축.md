...
> **문서 버전**: v1.0
> **작성일**: 2026-01-05
> **이전 문서**: [03_데이터베이스_설정.md](./03_데이터베이스_설정.md)
...
5. [벡터화 전처리 기준](#5-벡터화-전처리-기준)
6. [로컬 파이프라인 실행](#6-로컬-파이프라인-실행)
7. [결과 검증](#7-결과-검증)
8. [GitHub Actions CI 설정](#8-github-actions-ci-설정)
...
레포지토리 소스 코드 파일은 벡터화 전에 **의미 단위로 분할(chunking)**됩니다. 이를 통해 더 정확한 검색과 컨텍스트 유지가 가능합니다.

### 5.2 Chunking 규칙

#### 5.2.1 Chunk 크기
- **사이즈 자동 조정**: 파일 내용과 구조에 따라 최적 크기 자동 결정
- 임베딩 모델의 최대 토큰 제한을 고려하여 동적으로 조정

#### 5.2.2 오버랩(Overlap)
- **기본 오버랩: 8%**
- 인접한 chunk 간 8%의 내용이 겹치도록 설정
- 컨텍스트 연속성 유지 및 경계 정보 손실 방지

#### 5.2.3 의미 단위 분할
- **클래스 및 메소드 기준 분할**: 코드의 논리적 구조를 존중
  - 클래스 정의 단위로 분할
  - 메소드/함수 단위로 분할
  - 단, 길이가 긴 경우 추가 분리
- **분할 우선순위**:
  ...
  2. 메소드/함수 경계
  3. 주석 블록 경계
  4. 긴 메소드/함수는 내부에서 추가 분할

#### 5.2.4 긴 코드 처리
- **긴 메소드/함수**: 단일 메소드가 chunk 크기 제한을 초과하는 경우
  - 내부 로직 블록(if/for/while 등) 기준으로 추가 분할
  - 최소 단위는 유지하여 의미 보존
...
// 예시: TypeScript 파일
class UserService {
  // Chunk 1: 클래스 헤더 + 첫 번째 메소드
  async getUser(id: string): Promise<User> {
    // ... (8% 오버랩 포함)
  }
  
  // Chunk 2: 두 번째 메소드 (8% 오버랩으로 시작)
  async updateUser(id: string, data: UserData): Promise<User> {
    // ... 긴 메소드인 경우 내부에서 추가 분할
  }
}
```
...
각 chunk는 다음 메타데이터를 포함합니다:
- `path`: 파일 경로
- `chunkIndex`: chunk 인덱스 (0부터 시작)
- `totalChunks`: 전체 chunk 개수
- `fileType`: 파일 타입 (src, config, docs 등)
- `extension`: 파일 확장자
- `size`: 원본 파일 크기
